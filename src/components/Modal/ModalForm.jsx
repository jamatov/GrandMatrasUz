...
export default function ModalForm() {
  ...
  return (
    <section className='ModalForm' id='buyurtma'>
      <div className="container">
          <form className='contact-form row' onSubmit={sendForm} action="">
            <h3>{t("contactTittle")}</h3>
              <input className='col-12' type="text" placeholder={t("contactName")} />
              {/* {error.name[0] && <p style={{ color: 'red' }}>{error.name[1]}</p>} */}
              <i class='bx bx-phone-call'></i>
              <input autocomplete="off"   className='col-12 mb-3' maxlength="13" minlength="4" type="tel" data-intl-tel-input-id="1" placeholder={t("contactNumber")} 
                onChange={(e) => {
